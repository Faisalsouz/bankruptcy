...
def get_model(neurons_first_layer, neurons_second_layer, optimizer, loss, metrics, input_shape):
    model = Sequential()    
    model.add(LSTM(neurons_first_layer, input_shape=input_shape, return_sequences=True))
    model.add(LSTM(neurons_second_layer, input_shape=input_shape, return_sequences=True))
    model.add(Flatten())
